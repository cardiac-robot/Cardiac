...
import serial, sys, time, string, time, math
...
def ANG_euler(x,y,z,w):
    ...
    if t > 0.4999:
        ...
        sqy = y ** 2
        sqz = z ** 2
        heading = math.atan2(2 * y * w - 2 * x * z,
                            1 - 2 * sqy - 2 * sqz)
...
if __name__ == "__main__":
    ...
    comport = "/dev/tty.BlueRadios-COM0"
    
    reader = eMPL_packet_reader(comport)

    while 1:
        p1,p2,p3,p4 = reader.read()
        if (p1 == None) and (p2 == None) and (p3 == None) and (p4 == None):
            ...
            e1,e2,e3 = ANG_euler(p2,p3,p4,p1)
